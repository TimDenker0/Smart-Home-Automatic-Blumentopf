...
//----- BodenfeuchtigkeitsSensor -----
  ...
  #define BodenfeuchtigkeitsSensora2_Pinnus 33 
...
//----- NodeRed -----
  ...
  boolean dailyWarning = false;       //niedrieger Wasserstand

//----- Values -----
  float humiditaVallus;         //Luftfeuchtigkeit
  float humiditaVallusTemp;         //faengt 'nan' ab
  float temperaturaVallus;      //Raumtemperatur im Kasten
  float temperaturaVallusTemp;      //faengt 'nan' ab
  int mud1Vallus;               //gradueller Feuchtigkeitssensor fuer Erde
  int mud2Vallus;               //gradueller Feuchtigkeitssensor fuer Erde
  int waterNoWater1Value;       //Wassertankstand bei 1/3        (analog 1=Wasser, 0=keinWasser)
  int waterNoWater2Value;       //Wassertankstand bei 1/6        (analog 1=Wasser, 0=keinWasser)


  //----- Delay-Ersatz -----
  unsigned long previousMillis = 0; 
  const long interval = 2000;                 //2sec. "Delay" fuer Loop()

  unsigned long previousMillisTemperatur = 0; 
  const long intervalTemperatur = 10000;       //nach 5sec. neue Werte abfragen;

  unsigned long previousMillisReconnect = 0;
  const long intervalReconnect = 5000;        //nach 5sec. reconnect zu NodeRed wieder ausprobieren, falls noetig

  unsigned long previousMillisPumpe = 0;
  const long intervalPumpe = 2000;            //2sec. pumpen

  unsigned long previousMillisPumpe7 = 0;     
  const long intervalPumpe7= 15000;            //15sec. warten nach Pumpen fuer versickern
...
void loop() {
  ...
  unsigned long currentMillis = millis();               //Delay-Ersatz-Loop
...
//----- TemperatureSensor -----
  ...
  unsigned long currentMillis2 = millis();              //Delay-Ersatz-TempSensor
  if (currentMillis2 - previousMillisTemperatur >= intervalTemperatur) { 
    previousMillisTemperatur = currentMillis2; 

  // Reading temperature or humidity takes about 250 milliseconds!
  // Sensor readings may also be up to 2 seconds 'old' (its a very slow sensor)
  
  humiditaVallusTemp = dht.readHumidity();
  
  if(humiditaVallusTemp == humiditaVallusTemp){
    humiditaVallus = humiditaVallusTemp;
  }
  // Read temperature as Celsius (the default)
  
  temperaturaVallusTemp = dht.readTemperature();

  if(temperaturaVallusTemp == temperaturaVallusTemp){
    temperaturaVallus = temperaturaVallusTemp;
  }
...
//----- NodeRed -----
  ...
  else if(stunde == 1 && minu == 0 && sec < 10 && sec > 0){
    sendToday = false;
    dailyWarning = false;
  }
  if(dailyWarning == false){
...
void sendNodeRedWarning(int waterNoWaterValue1, int waterNoWaterValue2){
  if (waterNoWaterValue2 <= 200){
    client.publish("ALARM_Wasserstand_1/6", "Wassertank bei Füllstand von weniger als 1/6");
  } else if(waterNoWaterValue1 <= 200){
    client.publish("ALARM_Wasserstand_1/3", "Wassertank bei Füllstand von weniger als 1/3");
  }
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (currentMillisReconnect - previousMillisReconnect >= intervalReconnect) { 
...
void pumpenSteuerung(int Pumpe_ChUebergabe, int mud1Vallus, int mud2Vallus){
  ...
  //Serial.println(mudVallusAvg);


  unsigned long currentMillisPumpe7 = millis(); 
  int timeDifference = currentMillisPumpe7 - previousMillisPumpe7;
 if (timeDifference >= intervalPumpe7) {  
  
  if(mud1Vallus > 300 || mud2Vallus > 300){        //1900   TEST:
      pumpeValue = 6;
      pumpeAn = true;
      ...
      ledcWrite(Pumpe_ChUebergabe, pumpeValue);
      pumpeAn = false;
      previousMillisPumpe7 = currentMillisPumpe7;

      int test = currentMillisPumpe7 - previousMillisPumpe7;
...
void turbinenSteuerung(int Turbine_ChUebergabe, float temperaturaVallusUebergabe){
  ...
  //Serial.println(temperaturaVallusUebergabe);
    
  if(temperaturaVallusUebergabe > 28.0 && !pumpeAn){                //ca. bei 26*C im Gehaeuse (evtl. Sperre durch Pumpe)
    turbinenValue = 255;                 //100%
  } 
  else if(temperaturaVallusUebergabe > 23.0 && !pumpeAn){           //ca. bei 22*C im Gehaeuse (evtl. Sperre durch Pumpe)
